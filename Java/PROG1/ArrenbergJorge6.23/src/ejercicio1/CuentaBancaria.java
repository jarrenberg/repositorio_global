...
public class CuentaBancaria {
	...
	public CuentaBancaria(String iban, String titular) {
		if (ibanValido(iban) == false) {
			throw new IllegalArgumentException("El formato del IBAN introducido es incorrecto.");
		}
	...
	private boolean ibanValido(String iban) {
		...
		Matcher matcher;
		Pattern format = Pattern.compile("[E][S][0-9]{22}");
	...
	public void retirada(double cantidad) {
		...
		if ((saldo - cantidad) < MAXIMO_DESCUBIERTO) {
			throw new IllegalStateException(String
					.format("Operacion denegada. Se excede el maximo de descubierto (%.2f€)\n", MAXIMO_DESCUBIERTO));
		}
		if (numMovimientos > 99) {
			throw new IllegalStateException(
	...
	// Metodo que muestra por pantala cuando hay que avisar a hacienda si la cantidad introducida como parametro es mayor a 3000
	public void avisoHacienda(double cantidad) {
		if (cantidad > CANTIDAD_AVISO_HACIENDA) {
			System.out.println("AVISO: MOVIMIENTO MAYOR A 3000€. NOTIFICAR A HACIENDA.");
		}
	}
	...
	// Metodo que muestra el menu, permite interactuar con el y muestra los resultados de las acciones 
	public void menu(Scanner tecla) {
		int accion = 0;
		do {
