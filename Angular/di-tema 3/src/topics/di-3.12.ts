...
calendario.getPartidos().forEach((partido, index) => {
    ...
    console.log(`Fecha: ${partido.getFecha().toLocaleDateString()}`);
    console.log(`Hora: ${partido.getHora()}:00 horas`);
    console.log(`Recinto: ${partido.getRecinto()}`);
    console.log(`Pista: ${partido.getPista()}`);

    console.log('\nÁRBITRO:');
    const arbitro = partido.getArbitro();
    console.log(`  - ${arbitro.getNombre()} ${arbitro.getApellidos()}`);
    console.log(`  Nº Colegiado: ${arbitro.getNumeroColegiado()}`);
    console.log(`  Teléfono: ${arbitro.getTelefono()}`);
    console.log(`  Dirección: ${arbitro.getDireccion().calle}, ${arbitro.getDireccion().localidad}`);

    console.log('\nEQUIPOS PARTICIPANTES:');

    partido.getEquipos().forEach((equipo, equipoIndex) => {
        console.log(`\n  EQUIPO ${equipoIndex + 1}:`);
        equipo.getMiembrosEquipo().forEach((jugador, jugadorIndex) => {
            console.log(`    Jugador ${jugadorIndex + 1}:`);
            console.log(`      - Nombre: ${jugador.getNombre()} ${jugador.getApellidos()}`);
            console.log(`      - Género: ${jugador.getGenero()}`);
            console.log(`      - Categoría: ${jugador.getCategoria()}`);
            console.log(`      - Teléfono: ${jugador.getTelefono()}`);
            console.log(`      - Dirección: ${jugador.getDireccion().calle}, ${jugador.getDireccion().localidad}`);
        });
    });
